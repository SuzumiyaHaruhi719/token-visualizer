...
| 状态表现 | **像素重绘矢量 + 分部位逐状态动画**，跑工具显示工具名 | 12×8 SVG 分层；眨眼/踏步/挥手 |
| 多桌宠 | **每个活跃会话一只**，贴项目名，结束播离开动画 | 枚举 sessions/*.json 心跳 |
...
- **多会话 = 多桌宠**：枚举所有「心跳新鲜」的 `sessions/<pid>.json` = 当前活跃会话集合，
  **每个会话 spawn 一只独立桌宠**，各自用自己会话的 `status` + 自己 jsonl 尾部推导状态。
...
用户要求**每个状态、每个部位都动起来**。做法：把基准 Clawd 从参考图 **1:1 提取为 12×8 像素网格、
重绘为矢量（SVG `rect`）**，按 **身体 / 双眼 / 左右手(各1) / 四条腿** 分成独立图层；
用 CSS/Web 动画对各图层做关键帧（眨眼、踏步、挥手、歪头、挤压呼吸），得到真实的**分部位逐状态动画**。
纯矢量标记、**不依赖位图**（顺带规避了「服务器不服务图片→破图」的问题）。

- 资产：**SVG 矢量重绘**（12×8 网格，橙 `#da7757` / 黑眼 `#171717`），分部位图层；无需位图、天然透明、可无损缩放。
- 提取脚本已验证：从参考图自动求 bbox + 网格（结果 12×8）。
...
- **点击** → 打开完整仪表盘窗口。
- **右键** → 菜单：隐藏桌宠 / 切换置顶 / 退出。
- **每个活跃会话一只**，窗口下方**标项目名**（cwd 末段，如 CorePilot）区分；自动错位排布、各自可拖动。
- 会话结束 → 播**「离开」动画**后该窗口淡出销毁。
- 多显示器：记住所在屏与坐标。
...
| 多个活跃会话 | 各 spawn 一只桌宠（贴项目名）；会话消失则其桌宠播离开动画后离场 |
| 桌宠窗口数量过多 | 软上限（如 >8 只）时自动改紧凑簇排列，避免刷屏 |
